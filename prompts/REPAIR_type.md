...
- `global_transform_rules`, `review_flags`, and `assumptions` must be arrays
...
- `transform_actions`, `structural_transform_hints`, and `interpretation_hints` must be arrays
...
- if a code, category, or identifier is stored as integer or decimal, usually repair it by changing storage to string
- if `column_decisions` is empty, keep the summary consistent with “no reviewed columns required explicit override decisions”
- if a deprecated `supporting_structural_role` field appears in the invalid output, remove it unless a listed validation error explicitly requires preserving it for transition compatibility
